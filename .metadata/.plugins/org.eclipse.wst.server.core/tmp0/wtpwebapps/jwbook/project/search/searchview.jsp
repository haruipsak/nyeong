...
<main class="container">
  <div class="p-4 p-md-5 mb-4 rounded" style="background-color:#252221">
    <div class="col-md-10 px-0">
      <h1 class="display-4 fst-italic" style="color:white">홀리스틱? 프리미엄?</h1>
      <p class="lead my-3" style="color:white">냥바냥이 주성분을 기반으로 새로 정의했습니다.</p>
    </div>
  </div>
  ...
  <div class="row g-5">
    ...
    <%
						...
						ArrayList<Cat> list = search.getSearch(request.getParameter("searchText")); //여기서 검색하면서 가져오는 건데
						for (int i = 0; i < list.size(); i++) {
						%>
					<tr>
						<td><%=list.get(i).getCatfoodname()%></td>
						<td style="text-align:center"> <% if (list.get(i).getGrainfree() ==0) { %>
			     			       ○
			   			 <% } else { %>
			      		   X
						 <% } %>
						</td>
						<td style="text-align:center"><%=list.get(i).getFirst()%></td>
						<td style="text-align:center"><%=list.get(i).getCarbo()%></td>
						<td style="text-align:center"><%=list.get(i).getProtein()%></td>
						<td style="text-align:center"><%=list.get(i).getFat()%></td>
						<td style="text-align:center"><%=list.get(i).getFiber()%></td>
						 <td style="text-align:center"><% if (list.get(i).getCa() == null) { %>
			            알수없음
			     	   <% } else { %>
			            <%= list.get(i).getCa() %>
			       	 <% } %></td> 
						<td style="text-align:center"><%=list.get(i).getP()%></td>
        ...
        <div class="p-4">
          ...
          <ol class="list-unstyled mb-0">
           <li><a href="../dry.jsp" class="button-color basic-color" >냥바냥 건식 사료</a></li>
            ...
            <li><a href="../question.jsp" class="button-color basic-color">냥바냥 건의사항</a></li>
          </ol>
        </div>
      </div>
